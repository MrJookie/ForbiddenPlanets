...
int MicroPather::Solve( void* startNode, void* endNode, MP_VECTOR< void* >* path, float* cost )
{
	...
	*cost = 0.0f;
	...
	int openListElements = 1;
	//add max elements in case there is no solution, stop looking for whole 640x640 map 409600 iterations vs 5000 difference, huh? could be increased
	while (!open.Empty() && openListElements < 5000)
	{
